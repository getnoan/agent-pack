...
export const SHIPPED_TOOLS = ["grounding-check.mjs", "oss-sweeps.mjs", "pack-paths.mjs"];
...
/** Scripts a shipped worker EXECS, as paths under agents/. The closure walk follows JS imports
 *  only, so a subprocess target is invisible to it: market-research-refresh-worker runs both of
 *  these through execFileSync("python3", ...) and the pack shipped without them, so every search
 *  and scrape failed and returned null. Here for the reason SHIPPED_ASSETS is: the exporter ships
 *  them and the shipped sweeps must read them, and neither can import the other's copy. Both are
 *  stdlib-only; keep it that way — the pack has no requirements.txt and no install step. */
export const SHIPPED_SCRIPTS = ["scripts/firecrawl_search.py", "scripts/firecrawl_scrape.py"];
