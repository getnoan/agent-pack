...
const FLEET = path.join(REPO, "agents");
...
try {
  ...
  closure = new Set(readdirSync(FLEET).filter(f => f.endsWith(".mjs") && !f.startsWith("test-")));
  closureExact = false;
}
...
const { BUSINESS_LANES: LANE_MODULES } = await import("./optional-lane.mjs");
const loaded = [...readFileSync(path.join(FLEET, "reply-worker.mjs"), "utf8").matchAll(/loadLane\("([\w-]+)"/g)].map(m => m[1] + ".mjs");
...
const offenders = [];
for (const f of [...closure].sort()) offenders.push(...sweepCode(f, stripComments(readFileSync(path.join(FLEET, f), "utf8"))));
...
for (const f of [...closure].sort()) {
  ...
  rawCreates.push(...sweepRawContactCreate(f, readFileSync(path.join(FLEET, f), "utf8")));
}
...
const SHIPPED = new Set([...closure, ...DESIGN_PY.map(f => path.basename(f)), ...SHIPPED_TESTS, ...SHIPPED_TOOLS, "config.example.json"]);
