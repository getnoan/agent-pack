 ...
 *
 * Not hypothetical: the first version of PR #3 here edited two agents/ modules
 * and added a test. The next export would have reverted both edits and deleted
 * the test, with CI green throughout. This check moves that discovery from
 * "silently, on the next export" to "on the PR that introduces it".
 *
...
const CANONICAL_REPO = "getnoan/agent-pack";
...
console.error("     scripts/export-agent-pack.mjs upstream, then re-open this PR.");
process.exit(1);
